...
import torch
from model import SCAN, xattn_score_t2i, xattn_score_i2t, sim_t2i, sim_i2t, global_cos, ssim_i2t, ssim_t2i, salience_cos
...
def encode_data(model, data_loader, log_step=10, logging=print):
    ...
    for i, (images, captions, lengths, ids) in enumerate(data_loader):
        ...
        #img_emb(BN, regions, embed_size); glo_img(BN, glo_embed_size)
        img_emb, cap_emb, cap_len, glo_img, glo_txt, sal_img, sal_txt = model.forward_emb(images, captions, lengths, volatile=True)
        #print(img_emb)
        ...
        if glo_imgs is None:
            ...
            #glo_txts = np.zeros((len(data_loader.dataset), 11, glo_txt.size(2)))
        ...
        # glo_txts[ids, :max(lengths), :] = glo_txt.data.cpu().numpy().copy()
        ...
        model.forward_global_loss(glo_img, glo_txt, cap_len)
        model.forward_salience_loss(sal_img, sal_txt, cap_len)
...
evalrank(model_path="./runs/runX/checkpoint/model_best.pth.tar", data_path=None, split="f30k_11test")
